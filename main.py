...
loggFormat ="%(asctime)s %(levelname)s %(message)s %(name)s %(funcName)s %(lineno)d"
...
def db_connect(database_file=None):
    ...
    try:
        ...
        log.error('Error: {}'.format(err.args))
        return False
...
def get_last_run():
    ...
    log.info("Last run in DB %s" % data[0])
    ...
    if args.log_level != "debug":
        ...
        with con:
            try:
                ...
                log.debug("Last run updated in DB: %s" % timestamp)
            except sqlite3.Error as err:
                log.error('Sql error: {}'.format(err.args))
                return False
        ...
        log.debug("NOT REALY! Last run updated in DB: %s" % (timestamp))
...
def distance(string1, string2):
    ...
    if string1 is None or string2 is None:
        log.error('string1 can not be nothing')
        return -1
    if string1 is string2:
        return 0
    if len(string1) == 0 or len(string2) == 0:
       log.error('string1 content length is 0')
       return -1
...
def insert_video_to_db(videoId=None, timestamp=None, title=None, subscriptionId=None):
    ...
    if args.log_level != "debug":
        ...
        log.debug("NOT REALY!! Video %s (%s) from %s added to database" % (title, videoId, subscriptionId))
...
def get_videoId_from_db(videoId=None):
    ...
    log.debug("Checking %s in database" % (videoId))
    try:
    ...
    rows = query.fetchall()
    log.debug("count of rows in DB: %s" % len(rows))
    con.close()
    
    if len(rows) > 0:
        for row in rows:
            data = "{ \"id\": \"%s\" }" % row[0]
            log.debug("content of data before json.loads(): {}".format(data))
            try:
                data = json.loads(data)
            except json.JSONDecodeError as err:
                log.error("json error: {}".format(err))
                return 0
...
def insert_channel_to_db(channelId=None, channelTitle=None):
    ...
    if not args.local_json_files:
        ...
        with con:
            try:
                con.executemany(sql, data)
                log.debug("Channel %s with ID %s inserted into DB", channelId, channelTitle)
            except sqlite3.Error as err:
                log.error('Sql error: {}'.format(err.args))
                return False
        ...
        log.debug("Channel %s (%s) inserted into DB", channelTitle, channelId)
        return True
    else:
        log.debug("INSERT OR REPLACE INTO channel (id, title) VALUES(%s, %s)", channelId, channelTitle)
        return True
...
def insert_playlist_to_db(playlistId=None, playlistTitle=None):
    ...
    with con:
        try:
            con.executemany(sql, data)
            log.debug("Channel %s with ID %s inserted into DB", playlistTitle, playlistId)
        except sqlite3.Error as err:
            log.error('Sql error: {}'.format(err.args))
            return False
...
def get_playlist_from_db():
    ...
    log.debug("count: %s" % len(data))
    log.debug("results {}".format(json.dumps(data, indent=4)))
...
def insert_subscription_to_db(subscriptionId=None, subscriptionTitle=None, subscriptionTimestamp=None):
    ...
    if args.log_level != "debug":
        ...
        with con:
            try:
                con.executemany(sql, data)
                log.debug("Subscription %s with ID %s and timestamp: %s inserted into DB", subscriptionTitle, subscriptionId, subscriptionTimestamp)
            except sqlite3.Error as err:
                log.error('Sql error: {}'.format(err.args))
                return False
        ...
        log.debug("NOT REALY!! Subscription %s with ID %s and timestamp: %s inserted into DB", subscriptionTitle, subscriptionId, subscriptionTimestamp)        
...
def exit_func():
    ...
    log.info(f"Number of API calls made: {api_calls}", extra={"count": "api_calls", "value": api_calls})
    log.info(f"Number of subscriptions processed: {subscriptions_processed}", extra={"count": "subscriptions_processed", "value": subscriptions_processed})
    log.info(f"Number of subscriptions skiped: {subscriptions_skipped}", extra={"count": "subscriptions_skipped", "value": subscriptions_skipped})
    log.info(f"Number of videos added to playlist: {videos_added}", extra={"count": "videos_added", "value": videos_added})
    log.info(f"Number of videos skipped: {videos_skipped_count}", extra={"count": "videos_skipped_count", "value": videos_skipped_count})
    log.info(f"Number of Errors: {errors}", extra={"count": "errors", "value": errors})
    log.info(f"Number of Distances calculated: {dist_count}", extra={"count": "dist_count", "value": dist_count})
    log.info(f"Total distance acumulated: {dist_sum}", extra={"count": "dist_sum", "value": dist_sum})
    log.info(f"Avarage distance: {avg_dist}", extra={"count": "avg_dist", "value": avg_dist})
...
def get_subscriptions(credentials=None, nextPage=None):
    ...
    if args.local_json_files:
        ...
        if nextPage is None:
            log.debug("Getting all your subscription")
    ...
    if "nextPageToken" in subscriptions_response:
        log.debug("nextPageToken detected!")
...
def get_subscription_activity(credentials=None, channel=None, publishedAfter=None, nextPage=None):
    ...
    if args.local_json_files:
        ...
        if nextPage is None:
            log.debug("Getting activity for channelId: %s" % channel)
    ...
    if nextPage is None:
        log.debug("Total amount of activity for subscription %s: %s (from youtube API)" % (channel,activity_response["pageInfo"]["totalResults"]))
...
def get_video_duration(credentials=None, videoId=None):
    ...
    if "duration" in video_response["items"][0]["contentDetails"]:
        ...
        log.debug("%s duration is %s" % (videoId, video_time))
    else:
        video_time = 0
        log.warning("%s has no duration" % videoId)
...
def get_channel_id(credentials=None):
    ...
    if args.local_json_files:
        ...
        log.debug("Geting list of channels")
        ...
        try:
            ...
            log.debug("Respons: {}".format(json.dumps(channel_response, indent=4)))
            api_calls = api_calls + 1
        except HttpError as err:
            errors = errors + 1
            if err.resp.status in criticals:
    ...
    log.debug("channel_response: {}".format(json.dumps(channel_response, indent=4)))
    channel_list = channel_response["items"]

    channel_list = jq.all('.[] | { "title": .snippet.title, "id": .id }', channel_list)
    log.debug("Final channel list: {}".format(json.dumps(channel_list, indent=4)))
    log.debug("Final channel list count: %s" % len(channel_list))
...
def get_user_playlists(credentials=None, channelId=None, nextPage=None):
    ...
    if args.local_json_files:
        ...
        if nextPage is None:
            log.debug("Getting all your playlists")
        ...
        try:
            ...
            log.debug("playlists_response is of type %s and items count %s" % (type(user_playlists_response),len(user_playlists_response)))
            api_calls = api_calls + 1
        except HttpError as err:
            errors = errors + 1
            if err.resp.status in criticals:
    ...
    log.debug("plist_dict content: {}".format(json.dumps(plists_dict, indent=4)))
...
def get_playlist(credentials=None, channelId=None, playlistId=None, nextPage=None):
    ...
    if args.local_json_files:
        ...
        if nextPage is None:
            log.debug("Getting playlist %s" % playlistId)
    ...
    if nextPage is None:
        log.debug("Total amount of playlists: %s (from youtube API)" % playlist_response["pageInfo"]["totalResults"])
...
def main():
    ...
    if not playlist_from_db:
        ...
        log.debug("Playlists on channel: %s" % len(user_playlists_refined), extra={"count": "channel_playlists", "value": user_playlists_refined})
    ...
    ignore_subscriptions_list = get_file_list(args.youtube_subscription_ignore_file, "ignore subscription")
    ignore_video_list = get_file_list(args.youtube_video_ignore_file, "ignore video")
    ignore_word_list = get_file_list(args.youtube_words_ignore_file, "ignore word")
    log.debug(f"Subscriptions on ignore-list: {len(ignore_subscriptions_list)}", extra={"count": "ignore_subscriptions_list", "value": len(ignore_subscriptions_list)})
    log.debug("Subscripotions on ignore-list: {}".format(ignore_subscriptions_list))
    log.debug(f"Videos on ignore-list: {len(ignore_video_list)}", extra={"count": "ignore_video_list", "value": len(ignore_video_list)})
    log.debug("Videos on ignore-list: {}".format(ignore_video_list))
    log.debug(f"Words on ignore-list: {len(ignore_word_list)}", extra={"count": "ignore_word_list", "value": len(ignore_word_list)})
    log.debug("Words on ignore-list: {}".format(ignore_word_list))
    log.info("Last script run: %s" % (db_last_run))
    
    log.debug(f"Subscriptions on selected channel: {len(subscriptions_refined)}", extra={"count": "ignore_video_list", "value": len(subscriptions_refined)})
    ...
    for subs in subscriptions_refined:
        ...
        log.info("%s activity total count: %s" % (subs["title"], len(sub_activity_refined)), extra={"subscription_title": subs["title"], "subscription_id": subs["id"], "count": "activity_total_count", "value": len(sub_activity_refined)})
        log.debug("sub_activity_refined: {}".format(json.dumps(sub_activity_refined, indent=4)))
        
        a=0
        for activity in sub_activity_refined:
            log.debug("%s - Processing %s (%s)" % (subs["title"], activity["title"], activity["videoId"]), extra={"subscription_title": subs["title"], "subscription_id": subs["id"]})
            ...
            if activity["videoId"] in ignore_video_list:
                log.warning(f"Video {activity["videoId"]} is in video ignore list. Skipping", extra={"subscription_title": subs["title"], "subscription_id": subs["id"]})
                continue
            
            if results == 0:
                compare = compare_title_with_db_title(activity["title"])
                log.debug("COMPARE RESULTS: %s" % compare, extra={"subscription_title": subs["title"], "subscription_id": subs["id"]})
                ...
                if compare:
                    ...
                    if youtube_minimum_length != 0 and video_length >= youtube_minimum_length:
                        ...
                        if youtube_maximum_length != 0 and video_length <= youtube_maximum_length:
                            ...
                            videos_skipped_count = videos_skipped_count + 1
                            log.warning("Video maximum lenght is to long (duration: %s, maximum length: %s)" % (video_length, youtube_maximum_length), extra={"subscription_title": subs["title"], "subscription_id": subs["id"]})
                            continue
                        
                    else:
                        videos_skipped_count = videos_skipped_count + 1
                        log.warning("Video minimum lenght to short (duration: %s, minimum length: %s)" % (video_length, youtube_minimum_length), extra={"subscription_title": subs["title"], "subscription_id": subs["id"]})
                        continue
                    
                else:
                    videos_skipped_count = videos_skipped_count + 1
                    log.warning("COMPARE: %s - Video %s (%s) already in database or playlist" % (subs["title"], activity["title"], activity["videoId"]), extra={"subscription_title": subs["title"], "subscription_id": subs["id"]})
                    continue
            else:
                videos_skipped_count = videos_skipped_count + 1
                log.warning("get_videoId_from_db: %s - Video %s (%s) already in database or playlist %s" % (subs["title"], activity["title"], activity["videoId"], user_playlist["title"]), extra={"subscription_title": subs["title"], "subscription_id": subs["id"]})
                continue

            a=a + 1
            if args.youtube_activity_limit != 0 and a >= args.youtube_activity_limit:
                log.error("%s - YouTube activity limit reached! exiting activity loop" % (subs["title"]), extra={"subscription_title": subs["title"], "subscription_id": subs["id"]})
                break
        ...
        if args.youtube_subscription_limit != 0 and s >= args.youtube_subscription_limit:
            log.error("YouTube subscription limit reached! exiting subscription loop", extra={"subscription_title": subs["title"], "subscription_id": subs["id"]})
            break
    ...
    if errors == 0:
        if len(sub_activity_refined) > 0:
            ...
            log.warning("Last run timestamp not set because of no activity. (%s)" % errors, extra={"subscription_title": subs["title"], "subscription_id": subs["id"]})
    else:
        log.error("Last run timestamp not set because of errors! (%s)" % errors, extra={"subscription_title": subs["title"], "subscription_id": subs["id"]})
