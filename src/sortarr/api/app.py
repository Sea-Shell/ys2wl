...
import os
from sortarr.config import load_settings, Settings
...
from sortarr.api.routes import auth as auth_routes
...
async def lifespan(app: FastAPI) -> AsyncIterator:
    ...
    app.state.sortarr = state
    log.info("sortarr service started")
    yield
    ...
    if state.db_con:
        state.db_con.close()
    log.info("sortarr service stopped")


def create_app() -> FastAPI:
    app = FastAPI(
        title="sortarr",
        description="Sortarr - YouTube Subscription Router",
        version="2.0.0",
        lifespan=lifespan,
    )
