...
from ys2wl.db.connection import get_connection
...
def insert_video(con: sqlite3.Connection, video_id: str, timestamp: str, title: str,
                 subscription_id: str, playlist_id: str = "", duration_seconds: int = 0,
                 route_rule: str = "") -> bool:
    try:
        con.execute(
            "INSERT OR REPLACE INTO videos (videoId, timestamp, title, subscriptionId, playlistId, duration_seconds, route_rule) VALUES (?, ?, ?, ?, ?, ?, ?)",
            (video_id, timestamp, title, subscription_id, playlist_id, duration_seconds, route_rule),
        )
        return True
...
def insert_channel(con: sqlite3.Connection, channel_id: str, title: str) -> bool:
    try:
        con.execute("INSERT OR REPLACE INTO channel (id, title) VALUES (?, ?)", (channel_id, title))
        return True
    except sqlite3.Error as err:
...
def insert_playlist(con: sqlite3.Connection, playlist_id: str, title: str) -> bool:
    try:
        con.execute("INSERT OR REPLACE INTO playlist (id, title) VALUES (?, ?)", (playlist_id, title))
        return True
    except sqlite3.Error as err:
...
# -- subscription --
def insert_subscription(con: sqlite3.Connection, sub_id: str, title: str, timestamp: str) -> bool:
    try:
        con.execute(
...
def get_subscription_timestamp(con: sqlite3.Connection, sub_id: str) -> Optional[str]:
    cursor = con.execute("SELECT timestamp FROM subscription WHERE id = ? LIMIT 1", (sub_id,))
    row = cursor.fetchone()
...
def set_last_run(con: sqlite3.Connection, timestamp: str) -> bool:
    try:
        con.execute("INSERT OR REPLACE INTO last_run (id, timestamp) VALUES (1, ?)", (timestamp,))
        return True
    except sqlite3.Error as err:
...
def create_routing_rule(con: sqlite3.Connection, name: str, priority: int, field: Optional[str],
                        operator: str, pattern: Optional[str], dest_playlist_id: str,
                        dest_playlist_title: str) -> Optional[int]:
    ...
    try:
        cursor = con.execute(
            ...
            "VALUES (?, ?, ?, ?, ?, ?, ?, 1, ?, ?)",
            (name, priority, field, operator, pattern, dest_playlist_id, dest_playlist_title, now, now),
        )
...
def update_routing_rule(con: sqlite3.Connection, rule_id: int, **kwargs) -> bool:
    allowed = {"name", "priority", "field", "operator", "pattern", "destination_playlist_id", "destination_playlist_title", "enabled"}
...
# -- pipeline_runs --
def create_pipeline_run(con: sqlite3.Connection, trigger: str = "scheduled") -> Optional[int]:
    now = datetime.now(timezone.utc).isoformat()
    try:
        cursor = con.execute(
...
def finish_pipeline_run(con: sqlite3.Connection, run_id: int, summary: dict) -> bool:
    ...
    try:
        con.execute(
            ...
            (now, summary.get("status", "completed"), summary.get("subscriptions_processed", 0),
             summary.get("subscriptions_skipped", 0), summary.get("videos_added", 0),
             summary.get("videos_skipped", 0), summary.get("errors", 0),
             summary.get("error_message", ""), run_id),
        )
        return True
    except sqlite3.Error as err:
...
def set_config(con: sqlite3.Connection, key: str, value: str) -> bool:
    try:
        con.execute("INSERT OR REPLACE INTO app_config (key, value) VALUES (?, ?)", (key, value))
        return True
    except sqlite3.Error as err:
